...
if __name__ == "__main__":
    ...
    # parser.add_argument('--input_image_1', type=str, required=True, help='Path to the input image or directory')
    # parser.add_argument('--input_image_2', type=str, required=True, help='Path to the input image or directory')
    parser.add_argument("--image_paths", type=str, nargs="+", required=True, help="Paths to input images (e.g., --image_paths img1.png img2.png)")
    ...
    # # image_tensor = torch.stack(images, dim=0).unsqueeze(0)  # shape [1, N, 3, H, W]
    # image = Image.open(args.image_paths[0]).convert('RGB')
    ...
    output_images.extend(output_image)

    for i, output_image in enumerate(tqdm(output_images, desc="Saving images")):
        output_image.save(os.path.join(args.output_dir, os.path.basename(args.image_paths[i])))
